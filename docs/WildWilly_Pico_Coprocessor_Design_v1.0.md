...
| ⚠ **`0x60` = LEFT motor board, `0x61` = RIGHT** — FALSE, reversed by M-1 on 2026-09-18 | `config.py:29` has the measured truth (`MOTORKIT_LEFT_ADDR=0x61`, `MOTORKIT_RIGHT_ADDR=0x60`). **`CLAUDE.md` still asserts the old mapping twice** — in the roll-call table and in "Contradictions resolved … recorded so nobody re-opens them", which is the worst place for it: it tells a future reader the question is closed while stating the reversed answer |
| ⚠ **BNO085 RST lands on MCP23017 GPB4** | `config.py:114`, `sensors.py:120`, Master Hardware Design §6.3's connection table, §9 row 9. True today; **§3.2 of this document moves it to GP7** when the expander leaves, which also removes the init-ordering dependency (expander must be up before the IMU can be reset) |
